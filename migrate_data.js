...
let GeneralIsRunning = false

module.exports = function (complete) {

  if (GeneralIsRunning) {
    console.log('General report is still running')
    GeneralIsRunning = true
    return
  }
  GeneralIsRunning = true
  console.log('start')

  // extract all models with 'general' tag
  let models = Object.getOwnPropertyNames(report_models.all_models)
  let now = moment().format('')
  let models_index = []

  for (let rm in models) {
    if (models[rm].indexOf('general_') > -1) models_index.push(rm)
  }
  ...
  function json_key(object, key, k) {
    ...
    if (object === null) {
      ...
      if (
        ...
        if (model_data_type[k].indexOf('TIMESTAMP') !== -1) {
          return moment(
    ...
    if (typeof pg_create_res.rows[0] === 'undefined') {
      ...
      if (updated_at_flag) {
        latest_update_date = moment(pg_update_res.rows[0].updated_at)
          .add(1, 'seconds')
          .toISOString()
        console.log(latest_update_date)
      }

      latest_create_date = moment(pg_create_res.rows[0].created_at)
        .add(1, 'seconds')
        .toISOString()
      console.log(latest_create_date)
    }
    cb()
  }
